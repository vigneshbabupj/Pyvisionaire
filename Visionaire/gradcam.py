...
def plot_grad_cam_all_layers(model,classes,test_loader,device,num_img,savefig=False,*save_dir):
  ...
  for id in range(len(sample_images)):
    ...
    ax[id,0].set_title(f" Predicted:{classes[sample_pred[id]]} Target:{classes[sample_target_class[id]]}",color='red')
    ax[id,0].axis('off')
...
def plot_grad_cam_last_layer(model,classes,test_loader,device,num_img,savefig=False,*save_dir):
  ...
  for id in range(len(sample_images)):
    ...
    ax[r,c].set_title(f" Predicted:{classes[sample_pred[id]]} Target:{classes[sample_target_class[id]]}",color='red')
    ax[r,c].axis('off')
